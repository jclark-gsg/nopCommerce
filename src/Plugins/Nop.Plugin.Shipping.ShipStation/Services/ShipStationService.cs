...
namespace Nop.Plugin.Shipping.ShipStation.Services
{
    public partial class ShipStationService : IShipStationService
    {
        ...
        public const string API_URL = "https://ssapi.shipstation.com/";
            ...
            ShipStationSettings shipStationSettings)
        {
            ...
            var clientId = _shipStationSettings.ApiKey;
            var clientSecret = _shipStationSettings.ApiSecret;

            var encodedCredentials = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{clientId}:{clientSecret}"));
            _authorizationHeader = new AuthenticationHeaderValue("Basic", encodedCredentials);
        }
        ...
        {
            var client = _httpClientFactory.CreateClient(nameof(ShipStationService));
        ...
        protected virtual async Task<IList<ShipStationServiceRate>> GetRatesAsync(GetShippingOptionRequest getShippingOptionRequest, string carrierCode)
        {
            ...
            var responseData = await client.PostAsync(LIST_RATES_CMD, new StringContent(JsonConvert.SerializeObject(postData), new MediaTypeHeaderValue(CONTENT_TYPE)));
        ...
        {
            var rez = await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(_carriersCacheKey), async () =>
            {
                var data = await SendGetRequestAsync(LIST_CARRIERS_CMD);
        ...
        protected virtual async Task<IList<Service>> GetServicesAsync()
        {
            ...
            {
                var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(_serviceCacheKey, carrier.Code);

                var data = await _staticCacheManager.GetAsync(cacheKey, async () => await SendGetRequestAsync(string.Format(LIST_SERVICES_CMD, carrier.Code)));
        ...
        protected class Service : IEqualityComparer<Service>
        {
            ...
            public bool Equals(Service first, Service second)
            {
                ...
                return first == null ? false : first.Code.Equals(second?.Code);
            }
